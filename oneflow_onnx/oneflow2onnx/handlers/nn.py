...
class LayerNorm:
    ...
    def Version_9(cls, ctx, node, **kwargs):
        ...
        mean = ctx.MakeNode("ReduceMean", [node.input_tensor_names[0]], op_name_scope=node.name, name="mean", dtypes=dtypes[0], attr={"axes": axes, "keepdims": True})
        ...
        variance = ctx.MakeNode("ReduceMean", [pow_node.output_tensor_names[0]], op_name_scope=node.name, name="mean", dtypes=dtypes[0], attr={"axes": axes, "keepdims": True})
        ...
        ctx.RemoveNode(node.name)
        ctx.MakeNode("Reciprocal", [normalized.output_tensor_names[0]], outputs=[node.output_tensor_names[0]], op_name_scope=node.name, name="rdenominator", dtypes=dtypes[0])
