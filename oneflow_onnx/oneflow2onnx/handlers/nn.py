...
class UpSampleNearest2D:
    ...
    def Version_11(cls, ctx, node, **kwargs):
        ...
        roi_node = ctx.MakeConst(
            oneflow._oneflow_internal.UniqueStr("roi"),
            np.array(roi).astype(np.float32),
        )
...
@flow_op(["rms_layer_norm"], flow_ibns=["hidden_states", "weight", "variance_epsilon"])
class RMSLayerNorm:
    @classmethod
    def Version_1(cls, ctx, node, **kwargs):
        dtypes = node.output_dtypes
...
@flow_op(["rms_norm"], flow_ibns=["x", "weight"])
class RMSNorm:
    @classmethod
    def Version_1(cls, ctx, node, **kwargs):
        dtypes = node.output_dtypes
